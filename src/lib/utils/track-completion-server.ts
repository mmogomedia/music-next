/**
 * Server-side track completion calculation utility
 * Fetches rules directly from database (no API calls)
 */
...
/**
 * Get completion rules from database (server-side)
 */
export async function getCompletionRulesServer(): Promise<
  TrackCompletionRule[]
> {
  try {
    ...
    if (rules.length > 0) {
      return rules.map(rule => ({
        ...
        field: rule.field,
        label: rule.label,
        category: rule.category as WeightCategory,
        weight: rule.weight,
    ...
    console.warn(
      'Failed to fetch completion rules from DB, using defaults:',
      error
    );
  }

  // Fallback to static config
  return FIELD_WEIGHTS.map((fw, index) => ({
    id: `static-${fw.field}`,
    field: fw.field,
    label: fw.label,
...
/**
 * Calculate track completion using dynamic rules (server-side)
 */
export async function calculateTrackCompletionServer(
  track: TrackEditorValues
): Promise<CompletionBreakdown> {
  const rules = await getCompletionRulesServer();
