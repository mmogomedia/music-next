...
const row = (over: Partial<FlemojiArticleRow> = {}): FlemojiArticleRow => ({ ...base, ...over });
...
describe("cluster role — the rule that would import the bug", () => {
  it("an article in NO cluster has NO role, despite the source saying SPOKE", () => {
    ...
    expect(mapClusterRole(row({ clusterId: null, clusterRole: "SPOKE" }))).toBeNull();
    expect(mapClusterRole(row({ clusterId: null, clusterRole: "PILLAR" }))).toBeNull();
    expect(mapFlemojiArticle(row()).cluster).toBeNull();
  });

  it("an article IN a cluster keeps its role, lower-cased", () => {
    expect(mapFlemojiArticle(row({ clusterId: "c1", clusterSlug: "mixing", clusterRole: "PILLAR" })).cluster)
      .toEqual({ slug: "mixing", role: "pillar" });
    expect(mapFlemojiArticle(row({ clusterId: "c1", clusterSlug: "mixing", clusterRole: "SPOKE" })).cluster)
  ...
  it("a clusterId the caller could not resolve yields no cluster, not a half one", () => {
    ...
    expect(mapFlemojiArticle(row({ clusterId: "c1", clusterSlug: null })).cluster).toBeNull();
  });
});
...
describe("references — a free Json column, narrowed not cast", () => {
  it("drops entries with no usable url", () => {
    ...
    const refs = mapReferences(
      row({ references: [{ url: "https://a.test", title: "A" }, { title: "no url" }, { url: "   " }, null, "nope"] }),
    );
    expect(refs).toEqual([{ url: "https://a.test", title: "A" }]);
  });

  it("keeps only known keys, and returns null rather than an empty array", () => {
    expect(mapReferences(row({ references: [{ url: "https://a.test", junk: 1 }] })))
...
describe("provenance is never inferred", () => {
  it("is unknown even when the row looks machine-made", () => {
    ...
    expect(mapFlemojiArticle(row({ references: [{ url: "https://a.test" }] })).provenance).toBe("unknown");
...
describe("field mapping", () => {
  it("renames body, coverImageUrl, targetKeywords and scheduledAt", () => {
    const m = mapFlemojiArticle(row({ scheduledAt: new Date("2026-05-01T00:00:00Z") }));
  ...
  it("carries authorName only when the caller resolved it", () => {
    ...
    expect(mapFlemojiArticle(row({ authorName: "Tawanike" })).authorName).toBe("Tawanike");
  });
  ...
  it("NEVER emits readTime — the target derives it", () => {
    ...
    expect(JSON.stringify(mapFlemojiArticle(row({ readTime: 7 })))).not.toContain("readTime");
  });
...
describe("extras", () => {
  it("carries the Flemoji-only columns", () => {
    const m = mapFlemojiArticle(
      row({ toolSlugs: ["bpm"], timelinePostId: "t1", socialImages: [{ platform: "ig" }] }),
    );
  ...
  it("records that an embedding existed, WITHOUT the vector", () => {
    ...
    const m = mapFlemojiArticle(row({ embeddingUpdatedAt: new Date("2026-01-09T00:00:00Z") }));
    expect(m.extras.flemoji).toEqual({ embeddingUpdatedAt: "2026-01-09T00:00:00.000Z" });
