...
export default function ChatWelcomePlaceholder({ province, genre }: ChatWelcomePlaceholderProps) {
  ...
  const FeaturedTrackCard = ({ track, index }: { track: Track; index: number }) => {
      ...
      >
        ...
        <div className='relative aspect-square overflow-hidden bg-gradient-to-br from-blue-50 via-purple-50 to-green-50 dark:from-blue-950/40 dark:via-purple-950/30 dark:to-green-950/40'>
          ...
          <div className={`absolute inset-0 flex items-center justify-center transition-all duration-200 ${
            isHovered || isCurrentlyPlaying ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
          }`}>
            <div className={`w-16 h-16 rounded-full backdrop-blur-sm flex items-center justify-center shadow-2xl ring-2 transition-all ${
              isCurrentlyPlaying
              ...
              {isCurrentlyPlaying ? (
                ...
                <svg className='w-8 h-8 text-blue-600 dark:text-blue-400 ml-0.5' fill='currentColor' viewBox='0 0 24 24'>
              ...
              <div className='flex items-center gap-1 px-2 py-0.5 bg-blue-500/90 dark:bg-blue-600/90 backdrop-blur-sm rounded-full'>
                {isCurrentlyPlaying && (
                  <div className='flex gap-0.5 items-end h-3'>
                    <span className='w-1 h-3 bg-white rounded-full' style={{ animation: 'music-bounce 0.6s ease-in-out infinite' }}></span>
                    <span className='w-1 h-4 bg-white rounded-full' style={{ animation: 'music-bounce 0.6s ease-in-out 0.1s infinite' }}></span>
                    <span className='w-1 h-3 bg-white rounded-full' style={{ animation: 'music-bounce 0.6s ease-in-out 0.2s infinite' }}></span>
                  </div>
                )}
                <span className='text-[9px] font-bold text-white uppercase'>Now</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className='p-4'>
          <div className='mb-2.5'>
            <div className={`text-sm font-black truncate mb-1 transition-colors duration-200 flex items-center gap-1.5 ${
              isCurrentTrack 
              ...
              {track.title}
              {isCurrentlyPlaying && (
                <svg className='w-3.5 h-3.5 text-blue-500 animate-pulse' fill='currentColor' viewBox='0 0 24 24'>
                  <path d='M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z' />
                </svg>
              )}
            </div>
  ...
  const TopTenTrackCard = ({ track, index }: { track: Track; index: number }) => (
    <div 
  ...
  const PlaceholderCard = ({ isFeatured = false }: { isFeatured?: boolean }) => (
    <div className={`${isFeatured ? 'rounded-xl' : 'rounded-xl'} bg-white dark:bg-slate-800 border border-gray-200/80 dark:border-slate-700/80 ${isFeatured ? 'aspect-square' : 'h-20'} animate-pulse overflow-hidden`}>
      <div className={`${isFeatured ? 'w-full aspect-square bg-gray-200 dark:bg-slate-700' : 'w-14 h-14 m-3 rounded-lg bg-gray-200 dark:bg-slate-700'}`} />
      {isFeatured && (
    ...
    <div className='space-y-8'>
      ...
      {(province || genre) && (
        <div className='flex items-center gap-2 flex-wrap justify-center'>
          {province && <Chip size='sm' variant='flat' color='primary'>{province}</Chip>}
          {genre && <Chip size='sm' variant='flat' color='success'>{genre}</Chip>}
        </div>
      )}
          ...
          <div className='flex items-center justify-between mb-5'>
            <div className='flex items-center gap-3'>
              <div className='flex items-center gap-2 px-3.5 py-1.5 bg-gradient-to-r from-blue-500/10 to-purple-500/10 dark:from-blue-500/20 dark:to-purple-500/20 border border-blue-300/30 dark:border-blue-700/30 rounded-full shadow-sm'>
                ...
                <span className='text-xs font-bold text-blue-700 dark:text-blue-300 uppercase tracking-wider'>Featured</span>
              </div>
            </div>
          </div>
          ...
          {loading ? (
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className='flex items-center gap-4 bg-white dark:bg-slate-800 border border-gray-200/80 dark:border-slate-700/80 rounded-lg p-3 animate-pulse overflow-hidden'>
            ...
            <div className='text-center py-10'>
              ...
              <p className='text-sm font-medium text-gray-500 dark:text-gray-400'>No featured tracks available</p>
            </div>
          )}
        </div>
      </section>
