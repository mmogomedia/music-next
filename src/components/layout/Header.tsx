...
export default function Header() {
  ...
  return (
    ...
    >
      <NavbarContent>
        <NavbarMenuToggle aria-label={isMenuOpen ? 'Close menu' : 'Open menu'} className="sm:hidden text-gray-600" />
        <NavbarBrand>
      ...
      <NavbarContent justify="end">
        <NavbarItem className="hidden md:flex w-60">
          <Input
            ...
            variant="bordered"
            classNames={{ 
              inputWrapper: 'bg-gray-50 border-gray-200 hover:border-purple-300 focus-within:border-purple-500 shadow-sm',
          ...
          <>
            ...
            <NavbarItem>
              <Dropdown placement="bottom-end">
                ...
                <DropdownMenu aria-label="Profile Actions" variant="flat" className="backdrop-blur-xl">
                  <DropdownItem key="profile" className="h-14 gap-2">
                    <p className="font-semibold">Signed in as</p>
                    <p className="font-semibold">{session.user?.email}</p>
                  </DropdownItem>
                  <DropdownItem key="logout" color="danger" onPress={() => signOut()}>Sign out</DropdownItem>
                </DropdownMenu>
              </Dropdown>
            </NavbarItem>
          </>
      ...
      <NavbarMenu>
        <div className="px-4 py-2">
          <Input size="sm" placeholder="Search..." variant="bordered" classNames={{ inputWrapper: 'bg-content2' }} />
        </div>
        {menuItems.map((item) => (
        ...
        {session ? (
          <>
            ...
            <NavbarMenuItem key="m-signout">
              <button className="w-full text-left" onClick={() => signOut()}>Sign out</button>
            </NavbarMenuItem>
          </>
        ) : (
          <>
            ...
            <NavbarMenuItem key="m-register">
              <Link className="w-full" href="/register">Sign Up</Link>
            </NavbarMenuItem>
          </>
        )}
      </NavbarMenu>
