...
async function testStrengthScoring() {
  ...
  try {
    ...
    if (artistCount === 0 || trackCount === 0) {
      console.log(
        '❌ No artists or tracks found. Please run setup-test-data.js first.'
      );
      return;
    }
    ...
    console.log(
      `   - Testing with artist: ${testArtist.artistName} (${testArtist.id})`
    );

    // Test API endpoint
    const response = await fetch(
      `http://localhost:3000/api/stats/artist/${testArtist.id}/strength?timeRange=7d`,
      {
        method: 'GET',
        headers: {
    ...
    if (response.ok) {
      ...
      console.log(
        `   - Overall Score: ${data.data.scores.overallScore.toFixed(2)}`
      );
      console.log(
        `   - Engagement: ${data.data.scores.engagementScore.toFixed(2)}`
      );
      console.log(`   - Growth: ${data.data.scores.growthScore.toFixed(2)}`);
      console.log(`   - Quality: ${data.data.scores.qualityScore.toFixed(2)}`);
      console.log(
        `   - Potential: ${data.data.scores.potentialScore.toFixed(2)}`
      );
    } else {
    ...
    const topResponse = await fetch(
      'http://localhost:3000/api/stats/artists/top?timeRange=7d&limit=10',
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (topResponse.ok) {
      ...
      if (topData.data.artists.length > 0) {
        console.log(
          `   - Top artist: ${topData.data.artists[0].artistName} (Score: ${topData.data.artists[0].overallScore.toFixed(2)})`
        );
      }
    } else {
    ...
    if (storedScores.length > 0) {
      ...
      storedScores.forEach((score, index) => {
        console.log(
          `   ${index + 1}. ${score.artist.artistName}: ${score.overallScore.toFixed(2)} (${score.timeRange})`
        );
      });
    } else {
      console.log(
        '⚠️  No stored strength scores found. Run batch calculation to generate scores.'
      );
    }
    ...
    const batchResponse = await fetch(
      'http://localhost:3000/api/stats/batch-calculate',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    console.log('\n📋 Next Steps:');
    console.log(
      '   1. Check the admin dashboard for the new Strength Scoring tab'
    );
    ...
    console.log(
      '   4. Use the strength scores for artist discovery and scouting'
    );
  } catch (error) {
